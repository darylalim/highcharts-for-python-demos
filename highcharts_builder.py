...
#
# The scale is DESIGNED for this app rather than inherited with the chrome, and
# config.toml carries the full argument. The short version, because it is what a future
# edit here has to not undo: the template's scale was the Tailwind -400 step of eight
# hues, so every entry sat at L* 64-81 and HUE was the only channel carrying series
# identity. Measured with dichromacy simulated (Machado-Oliveira-Fernandes 2009), four of
# its 28 pairs fell under dE 20 in normal vision and six under dE 12 for deuteranopes —
# blue and violet, at slots 0 and 2 where a three-series chart puts them, came out
# **0.3 apart**. This scale's worst pair is 19.4 normal / 10.1 deuteranopia / 9.6
# protanopia / 10.3 tritanopia, bought by spreading L* where hue cannot separate.
#
# Slot 0 is deliberately UNCHANGED through that redesign: it is the most-loaded value in
# the module (the single-series default, _WATERFALL_SUM_COLOR, bullet's bar, dumbbell's
# after-state) and the Streamlit accent besides, so it stays the brand blue and stays
# bright. Freeing it was measured at ~1.1 dE of extra separation for a dimmer primary,
# and declined.
#
# test_no_series_colour_collides_with_the_chart_chrome is what keeps a chrome value from
# walking back into this scale — the incident that produced the template's own one-off
# deviation, and the reason the guard outlived the palette that motivated it.
#
...
# gridline slate "so a missing reading reads as 'no cell here' ... instead of as a low
# value on the ramp" — but at #0c4a6e the ramp's cold end and that null fill were dE00
# **8.7** apart, inside the confusable band, so the sentence was not true of the code it
# described. One stop up puts them 12.5 apart and makes it true. Pinned by the same
# membership+ordering rule, which the new stop satisfies unchanged.
_HEATMAP_GRADIENT = {"minColor": "#075985", "maxColor": "#7dd3fc"}
